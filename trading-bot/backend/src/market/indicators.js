const { RSI, MACD, SMA, EMA, BollingerBands, ATR } = require('technicalindicators');

/**
 * Computes the standard indicator set from an array of candles
 * (oldest -> newest, shape: {open,high,low,close,volume}).
 * Returns the latest value of each indicator plus enough context for
 * confluence scoring. Returns null if there isn't enough history yet.
 */
...
/**
 * Simple pivot-based support/resistance: local swing highs/lows over the
 * lookback window, clustered to the nearest ATR-scaled tolerance.
 */
function findSupportResistance(candles, lookback = 100, swingSize = 3) {
...
module.exports = { computeIndicators, findSupportResistance };
